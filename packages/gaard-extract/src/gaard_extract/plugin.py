...
from gaard_extract.api import LicenseHostService, create_router
...
def register_api(context: ExtensionContext) -> None:
    ...
    license_service = cast(LicenseHostService, require_service(context.services, "license"))

    context.registry.register_initializer(lambda: initialize_runtime(session_factory))
    context.registry.register_router(
        extension_id=context.extension_id,
        router=create_router(session_factory, datasource_service, license_service),
    )
